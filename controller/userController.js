...
module.exports.view_details = async (req, res, next) => {
    try {
        ...
        const userId = req.session.userId
        const user = await usermodel.findById(userId)
        res.render('user/productDetails', { products, product, user, count, countwish })
        ...
        if (user) {
            ...
            if (userCart != null) {
                ...
                if (proExist != -1) {
                    ...
                    await cartModel.updateOne({ user: userId, 'products.item': prodId },
                        {
            ...
            await cartModel.updateOne({ _id: cart, 'products.item': product },
                {
                    ...
                    const carts = await cartModel.findOne({ user: userId }).populate('products.item')
                    const total = carts.products.reduce((acc, cur) => (acc + cur.item.price * cur.quantity), 0)
        ...
        if (user) {
            ...
            if (wishlist) {
                ...
                if (productExist != -1) {
                    ...
                    const Exist = await wishlistModel.findOne({ user: userId }).populate('products')
...
module.exports.delete_address = async (req, res, next) => {
    try {
        ...
        await addressModel.updateOne({ user: userId }, { $pull: { address: { _id: address } } })
            .then((data) => {
...
module.exports.place_order = async (req, res, next) => {
    try {
        ...
        res.render('user/placeOrder', { total, address, user, count, countwish, product })
...
module.exports.placing_order = async (req, res, next) => {
    try {
        ...
        const userId = req.session.userId
        let cart = await cartModel.findOne({ user: userId }).populate('products.item')
        let products = cart.products
        let total = cart.products.reduce((acc, cur) => (acc + cur.item.price * cur.quantity), 0)
